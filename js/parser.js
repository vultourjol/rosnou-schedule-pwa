class ScheduleParser {
  ...
  processScheduleData(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // Строка с месяцами
      if (
        rowStr.includes("ФЕВРАЛЬ") ||
        rowStr.includes("МАРТ") ||
        rowStr.includes("АПРЕЛЬ")
      ) {
        monthsRowIdx = i;
      }
      ...
      if (
        ...
        (String(row[1]).includes(">") ||
          String(row[1]).toUpperCase().includes("НЕДЕЛ"))
      ) {
        weeksRowIdx = i;
      }
      ...
      if (
        row[1] &&
        String(row[1]).toUpperCase() === "ПОНЕДЕЛЬНИК" &&
        firstDayRowIdx === -1
      ) {
        firstDayRowIdx = i;
        break;
    ...
    for (let col = 2; col < (monthsRow ? monthsRow.length : 0); col++) {
      const cell = String(monthsRow[col] || "")
        .toUpperCase()
        .trim();
      if (monthNames[cell]) {
      ...
      if (isTeacher) {
        ...
        for (let col = 2; col < row.length; col++) {
          ...
          for (let prevRow = rowIdx - 1; prevRow >= firstDayRowIdx; prevRow--) {
            ...
            if (
              prevCol1 &&
              !/преп\.|проф\.|доц\.|ст\.\s*преп/i.test(prevCol1)
            ) {
          ...
          if (!exists) {
            ...
            console.log(
              `✅ Добавлено: ${dateKey} - ${subjectName} (${lessonType})`,
            );
          }
        }
      } else {
