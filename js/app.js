document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedSchedule) {
        ...
        if (lastUpdate) {
            console.log('📅 Последнее обновление:', lastUpdate.toLocaleString('ru-RU'));
        }
    } else {
    ...
    function renderTodaySchedule() {
        ...
        if (classes.length === 0) {
            todayLessons.innerHTML = '<div class="no-classes"><i class="ri-cup-line"></i> Занятий нет!</div>';
        } else {
            ...
            todayLessons.innerHTML = classes.map(lesson => {
